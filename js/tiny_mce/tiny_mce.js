(function(b) {
	...
	b.tinymce = b.tinyMCE = {
		...
		_init : function() {
			...
			r.isOpera = b.opera && opera.buildNumber;
			...
			function g(d) {
				if (d.src
					...
					if ((h = d.src.indexOf("?")) != -1) {
						r.query = d.src.substring(h + 1)
					}
					r.baseURL = d.src.substring(0, d.src.lastIndexOf("/"));
		...
		is : function(e, d) {
			...
			if (d == "array" && (e.hasOwnProperty && e instanceof Array)) {
				return true
			}
			return typeof (e) == d
		},
		...
		extend : function(j, h) {
			var g, f, d = arguments;
			for (g = 1, f = d.length; g < f; g++) {
				h = d[g];
				tinymce.each(h, function(e, i) {
					if (e !== c) {
						j[i] = e
					}
				})
		...
		trim : function(d) {
			return (d ? "" + d : "").replace(a, "")
		},
		create : function(l, d) {
			var k = this, e, g, h, i, f, j = 0;
			l = /^((static) )?([\w.]+)(:([\w.]+))?/.exec(l);
			h = l[3].match(/(^|\.)(\w+)$/i)[2];
			g = k.createNS(l[3].replace(/\.\w+$/, ""));
			if (g[h]) {
				return
			}
			if (l[2] == "static") {
			...
			g[h] = d[h];
			k.extend(g[h].prototype, d);
			if (l[5]) {
				e = k.resolve(l[5]).prototype;
				i = l[5].match(/\.(\w+)$/i)[1];
				f = g[h];
				if (j) {
					g[h] = function() {
				...
				g[h].prototype[h] = g[h];
				k.each(e, function(m, o) {
					g[h].prototype[o] = e[o]
				});
				k.each(d, function(m, o) {
					if (e[o]) {
						g[h].prototype[o] = function() {
							this.parent = e[o];
			...
			k.each(d["static"], function(m, o) {
				g[h][o] = m
			});
			if (this.onCreate) {
				this.onCreate(l[2], l[3], g[h].prototype)
			}
		},
		walk : function(g, e, h, d) {
			d = d || this;
			if (g) {
				if (h) {
					g = g[h]
				}
				tinymce.each(g, function(j, f) {
					if (e.call(d, j, f, h) === false) {
						return false
					}
					tinymce.walk(j, e, h, d)
				})
			}
		},
		createNS : function(g, f) {
			var e, d;
			f = f || window;
			g = g.split(".");
		...
		addUnload : function(h, g) {
			...
			if (!e.unloads) {
				function d() {
					...
					if (f) {
						for (k in f) {
							j = f[k];
							if (j && j.func) {
								j.func.call(j.scope, 1)
							}
						}
						if (b.detachEvent) {
							b.detachEvent("onbeforeunload", i);
							b.detachEvent("onunload", d)
						} else {
							if (b.removeEventListener) {
								b.removeEventListener("unload", d, false)
							}
						}
						e.unloads = j = f = w = d = 0;
						if (b.CollectGarbage) {
				...
				if (b.attachEvent) {
					b.attachEvent("onunload", d);
					b.attachEvent("onbeforeunload", i)
				} else {
					if (b.addEventListener) {
						b.addEventListener("unload", d, false)
					}
				}
				e.unloads = [ h ]
			} else {
		...
		removeUnload : function(g) {
			var d = this.unloads, e = null;
			tinymce.each(d, function(h, f) {
				if (h && h.func == g) {
					d.splice(f, 1);
					e = g;
					return false
		...
		explode : function(e, f) {
			return e ? tinymce.map(e.split(f || ","), tinymce.trim) : e
		},
		_addVer : function(e) {
			var d;
			if (!this.query) {
					...
					"tinymce.dom.DOMUtils",
					{
							...
							return this
									.run(
											...
											function(u) {
												...
												if (o) {
													for (r = u.childNodes.length - 1; r >= 0; r--) {
														p
																.insertAfter(
																		u.childNodes[r],
																		u)
						...
						replace : function(s, r, p) {
							...
							return q.run(r, function(t) {
								...
								if (q.fixPsuedoLeaks && t.nodeType === 1) {
						...
						nodeIndex : function(r, s) {
							var o = 0, q, p;
							if (r) {
								for (r = r.previousSibling, q = r; r; r = r.previousSibling) {
									p = r.nodeType;
									if (s && p == 3) {
										if (r.nodeValue.length > 0
												&& (q.nodeType != p || q.nodeValue.length === 0)) {
											o++
										}
									} else {
										o++
...
(function() {
	function a(e) {
		var d = this, i = "\uFEFF", b, h, g = e.dom;
		function c(k, j) {
			if (k && j) {
				if (k.item && j.item && k.item(0) === j.item(0)) {
					return 1
				}
				if (k.isEqual && j.isEqual && j.isEqual(k)) {
					return 1
				}
			}
			return 0
		...
		function f() {
			var n = e.getRng(), j = g.createRng(), k, l, p, o;
			l = n.item ? n.item(0) : n.parentElement();
			if (l.ownerDocument != g.doc) {
				return j
			}
			if (n.item || !l.hasChildNodes()) {
				j.setStart(l.parentNode, g.nodeIndex(l));
				j.setEnd(j.startContainer, j.startOffset + 1);
				return j
			}
			k = n.duplicate();
			p = e.isCollapsed();
			n.collapse();
			n
					.pasteHTML('<span id="_mce_start" style="display:none;line-height:0">\uFEFF</span>');
			if (!p) {
				k.collapse(false);
				k
						.pasteHTML('<span id="_mce_end" style="display:none;line-height:0">\uFEFF</span>')
			}
			function m(u) {
				var r, t, q, s;
				q = g.get("_mce_" + (u ? "start" : "end"));
				s = q.previousSibling;
				if (s && s.nodeType == 3) {
					r = s;
		...
		this.addRange = function(k) {
			var s, y, x = e.dom.doc, q = x.body, t, m, v, n, r, j, o, p, u, l;
			this.destroy();
			v = k.startContainer;
			n = k.startOffset;
			r = k.endContainer;
			...
			if (r.nodeType == 1 && r.hasChildNodes()) {
				p = r.childNodes.length - 1;
				if (j == 0) {
					l = 1;
					r = r.childNodes[0]
				} else {
					r = r.childNodes[Math.min(p, j - 1)];
					if (r.nodeType == 3) {
						j = r.nodeValue.length
					}
				}
			}
			if (v == r && v.nodeType == 1) {
				if (/^(IMG|TABLE)$/.test(v.nodeName) && n != j) {
					s = q.createControlRange();
					s.addElement(v)
				} else {
					s = q.createTextRange();
					if (!v.hasChildNodes() && v.canHaveHTML) {
						v.innerHTML = i
					}
				...
				if (n == j) {
					s.collapse(j <= k.endContainer.childNodes.length - 1)
				}
				s.select();
				s.scrollIntoView();
				return
			...
			if (r.nodeType == 3) {
				...
				y.moveToElementText(o);
				o.parentNode.removeChild(o);
				y.move("character", j);
				s.setEndPoint("EndToStart", y)
			} else {
				y.moveToElementText(r);
				y.collapse(!!l);
				s.setEndPoint("EndToEnd", y)
			}
			s.select();
			s.scrollIntoView()
		};
		...
		if (e.dom.boxModel) {
			(function() {
				var p = g.doc, k = p.body, m, n;
				p.documentElement.unselectable = true;
				function o(q, t) {
					var r = k.createTextRange();
					try {
						r.moveToPoint(q, t)
					} catch (s) {
						r = null
				...
				function l(r) {
					...
					if (r.button) {
						q = o(r.x, r.y);
						if (q) {
							if (q.compareEndPoints("StartToStart", n) > 0) {
								q.setEndPoint("StartToStart", n)
							} else {
								q.setEndPoint("EndToEnd", n)
							}
							q.select()
						}
					} else {
				...
				function j() {
					g.unbind(p, "mouseup", j);
					g.unbind(p, "mousemove", l);
					m = 0
				}
				g.bind(p, "mousedown", function(q) {
						...
						getBookmark : function(q, r) {
							...
							if (q == 2) {
								function k() {
									...
									function y(z, B) {
										var F = [], D, H, A = z[B ? "startContainer"
												: "endContainer"], E = z[B ? "startOffset"
												: "endOffset"], C, G = {};
										if (A.nodeType == 1
												&& A.hasChildNodes()) {
											H = A.childNodes.length - 1;
											G.exclude = (B && E > H)
													|| (!B && E == 0);
											if (!B && E) {
												E--
											}
											A = A.childNodes[E > H ? H : E];
											if (A.nodeType == 3) {
												E = B ? 0 : A.nodeValue.length
											}
										}
										if (A.nodeType == 3) {
											if (r) {
												for (D = A.previousSibling; D
							...
							if (l) {
								if (l.start) {
									...
									function g(s) {
										...
										if (o) {
											for (q = m, p = o.indexes.length - 1; p >= 0; p--) {
												q = q.childNodes[o.indexes[p]]
														|| q
											}
											if (s) {
												if (q.nodeType == 3 && o.offset) {
													f.setStart(q, o.offset)
												} else {
													if (o.exclude) {
														f.setStartAfter(q)
													} else {
			...
			.create(
					"tinymce.Editor",
					{
						...
						focus : function(q) {
							...
							if (!q) {
								if (!r
										&& (!b || p.selection.getNode().ownerDocument != p
												.getDoc())) {
									p.getWin().focus()
								}
							}
						...
						nodeChanged : function(r) {
							var p = this, q = p.selection, u = q.getNode()
									|| p.getBody();
							if (p.initialized) {
								r = r || {};
...
(function(a) {
	a.create("tinymce.UndoManager", {
		...
		add : function(d) {
			...
			j = g.data[g.index];
			if (j && j.content == d.content) {
				if (g.index > 0 || g.data.length == 1) {
					return null
				}
			}
			...
			if (h.custom_undo_redo_restore_selection) {
				d.bookmark = c = d.bookmark || e.selection.getBookmark(2, true)
			}
			if (g.index < g.data.length - 1) {
				if (g.index == 0) {
					g.data = []
				} else {
								...
								function q(t) {
									...
									t
											...
											.scrollTo(
													0,
													r
															.getPos(o.getRng().startContainer).y)
								}
								m.onKeyPress
										.add(function(s, t) {
											if (t.keyCode == 13
													&& (t.shiftKey || p.force_br_newlines)) {
												q(s);
												k.cancel(t)
											}
										})
...
(function(a) {
	a.Formatter = function(Q) {
		var H = {}, J = a.each, b = Q.dom, n = Q.selection, q = a.dom.TreeWalker, F = new a.dom.RangeUtils(
				b), c = Q.schema.isValid, A = b.isBlock, i = Q.settings.forced_root_block, p = b.nodeIndex, z = "\uFEFF", N = false, v = true, m, R, K;
		function j(T, S) {
			return b.getParents(T, S, b.getRoot())
		}
		function C() {
			if (!K || K.apply.length || K.remove.length) {
				K = {
					apply : [],
					remove : []
				}
		...
		function O(U, aa, W) {
			...
			function S(ae, ad) {
				ad = ad || ab;
				if (ae) {
					J(ad.styles, function(ag, af) {
						b.setStyle(ae, af, o(ag, aa))
					});
					J(ad.attributes, function(ag, af) {
						b.setAttrib(ae, af, o(ag, aa))
					});
					J(ad.classes, function(af) {
						af = o(af, aa);
						if (!b.hasClass(ae, af)) {
							b.addClass(ae, af)
						}
			...
			function ac(ae) {
				...
				F
						.walk(
								...
								function(ah) {
									var ai;
									function aj(ak) {
										var am = ak.nodeName.toLowerCase(), al = ak.parentNode.nodeName
												.toLowerCase();
										if (e(am, "br")) {
											ai = 0;
											if (ab.block) {
										...
										if (ab.wrapper && s(ak, U, aa)) {
											ai = 0;
											return
										}
										if (ab.block && !ab.wrapper && B(am)) {
											ak = b.rename(ak, ag);
											S(ak);
											ad.push(ak);
											ai = 0;
				...
				J(ad, function(aj) {
					...
					function ai(al) {
						var an, am;
						J(al.childNodes, function(ao) {
							if (ao.nodeType == 1 && !D(ao)) {
								an = ao;
								return N
							}
						});
					...
					if (ab.inline || ab.wrapper) {
						...
						J(X, function(al) {
							J(b.select(al.inline, aj), function(am) {
								P(al, aa, am, al.exact ? am : null)
							})
						});
						b.getParent(aj.parentNode, function(al) {
							if (s(al, U, aa)) {
								b.remove(aj, 1);
								aj = 0;
								return v
			...
			if (ab) {
				if (W) {
					...
					if (!n.isCollapsed() || !ab.inline) {
						Z = n.getBookmark();
						ac(l(n.getRng(v), X));
						n.moveToBookmark(Z);
						n.setRng(V(n.getRng(v)));
						Q.nodeChanged()
					} else {
						L("apply", U, aa)
					}
		...
		function u(U, ac, W) {
			var X = M(U), ae = X[0], ab, aa, T;
			function V(ah) {
				var ag;
				ag = a.grep(ah.childNodes);
				J(X, function(ai) {
					if (P(ai, ac, ah, ah)) {
						return N
					}
			...
			function Y(ag) {
				var ah;
				J(j(ag.parentNode).reverse(), function(ai) {
			...
			function ad(ag) {
				return S(Y(ag), ag, ag, true)
			}
			function Z(ai) {
				var ah = b.get(ai ? "_start" : "_end"), ag = ah[ai ? "firstChild"
						: "lastChild"];
				b.remove(ah, 1);
				return ag
			}
		...
		function s(V, S, X) {
				...
				if (aa) {
					if (aa.length === m) {
						...
						for (ab = 0; ab < aa.length; ab++) {
							if (ag === "attributes" ? b.getAttrib(ad, aa[ab])
									: G(ad, aa[ab])) {
								return v
							}
						}
			...
			if (Y && V) {
				for (U = 0; U < Y.length; U++) {
					W = Y[U];
					if (f(V, W) && Z(V, W, "attributes") && Z(V, W, "styles")) {
						if (T = W.classes) {
		...
		function g(U, X, W) {
			...
			if (n.isCollapsed()) {
				for (V = K.apply.length - 1; V >= 0; V--) {
					if (K.apply[V].name == U) {
						return true
					}
				}
				for (V = K.remove.length - 1; V >= 0; V--) {
					if (K.remove[V].name == U) {
						return false
					}
				}
				return S(n.getNode())
		...
		function d(S) {
			return S && S.nodeType === 3 && /^\s*$/.test(S.nodeValue)
		}
		function I(U, T, S) {
			var V = b.create(T, S);
		...
		function l(S, aa, V) {
			...
			function ab(ag, ah, ae, af) {
				...
				for (;;) {
					ai = ag.parentNode;
					if (ai == af || (!aa[0].block_expand && A(ai))) {
						return ag
					}
					for (ac = ai[ah]; ac && ac != ag; ac = ac[ae]) {
						if (ac.nodeType == 1 && !D(ac)) {
							return ag
						}
						if (ac.nodeType == 3 && !d(ac)) {
							return ag
						}
					}
					ag = ag.parentNode
			...
			if (aa[0].selector && aa[0].expand !== N) {
				function W(af, ae) {
					var ag, ah, ai;
			...
			if (aa[0].block || aa[0].selector) {
				function T(af, ae, ah) {
					var ag;
					if (!aa[0].wrapper) {
						ag = b.getParent(af, aa[0].block)
					}
					if (!ag) {
						ag = b.getParent(af.nodeType == 3 ? af.parentNode : af,
								A)
					}
					if (ag && aa[0].wrapper) {
						ag = j(ag, "ul,ol").reverse()[0] || ag
					}
					if (!ag) {
						ag = af;
						while (ag[ae] && !A(ag[ae])) {
		...
		function P(Y, X, V, S) {
			...
			if (Y.remove != "all") {
				...
				if (W && b.getAttrib(V, "style") == "") {
					V.removeAttribute("style");
					V.removeAttribute("_mce_style")
				}
				J(Y.attributes, function(ab, Z) {
					var aa;
					ab = o(ab, X);
					if (typeof (Z) === "number") {
						Z = ab;
					...
					if (!S || e(b.getAttrib(S, Z), ab)) {
						if (Z == "class") {
							ab = b.getAttrib(V, Z);
							if (ab) {
								aa = "";
								J(ab.split(/\s+/), function(ac) {
									if (/mce\w+/.test(ac)) {
										aa += (aa ? " " : "") + ac
									}
								});
		...
		function k(U, V) {
			...
			if (V.block) {
				if (!i) {
					function W(Y, X, Z) {
						Y = x(Y, X, Z);
						return !Y || (Y.nodeName == "BR" || A(Y))
					}
					if (A(U) && !A(S)) {
						if (!W(U, N) && !W(U.firstChild, v, 1)) {
							U.insertBefore(b.create("br"), U.firstChild)
						}
						if (!W(U, v) && !W(U.lastChild, N, 1)) {
							U.appendChild(b.create("br"))
						}
					}
				} else {
					if (S == b.getRoot()) {
						if (!V.list_block || !e(U, V.list_block)) {
							J(a.grep(U.childNodes), function(X) {
								if (c(i, X.nodeName.toLowerCase())) {
									if (!T) {
										T = I(X, i)
									} else {
										T.appendChild(X)
		...
		function D(S) {
			return S && S.nodeType == 1
					&& S.getAttribute("_mce_type") == "bookmark"
		}
		function r(W, V) {
			var S, U, T;
			function Y(ab, aa) {
				...
				function Z(ad) {
					var ae = {};
					J(b.getAttribs(ad), function(af) {
						var ag = af.nodeName.toLowerCase();
						if (ag.indexOf("_") !== 0 && ag !== "class"
								&& ag !== "style") {
							ae[ag] = b.getAttrib(ad, ag)
						}
					});
					return ae
				}
				...
				if (!ac(b.parseStyle(b.getAttrib(ab, "style")), b.parseStyle(b
						.getAttrib(aa, "style")))) {
					return N
				}
				return v
			...
			if (W && V) {
				...
				if (Y(W, V)) {
					...
					b.remove(V);
					J(a.grep(V.childNodes), function(Z) {
						W.appendChild(Z)
					});
					return W
				}
		...
		function B(S) {
			return /^(h[1-6]|p|div|pre|address)$/.test(S)
		}
		function E(T, W) {
			var S, V, U;
		...
		function L(aa, V, Z) {
			var W, T, Y = n.getNode().parentNode, ac = Q.getDoc(), X = "mceinline", af = [
					"onKeyDown", "onKeyUp", "onKeyPress" ], U = K[aa], ab = K[aa == "apply" ? "remove"
					: "apply"];
			...
			function ad(ag) {
				J(K.apply.reverse(), function(ah) {
					O(ah.name, ah.vars, ag)
				});
				J(K.remove.reverse(), function(ah) {
					u(ah.name, ah.vars, ag)
				});
				b.remove(ag, 1);
				C()
			}
			function S(ag) {
				return ag.face == X || ag.style.fontFamily == X
			}
			ae();
			ac.execCommand("FontName", false, X);
			J(b.select("font,span", Y), function(ah) {
				var ag;
				if (S(ah)) {
					ag = n.getBookmark();
					ad(ah);
			...
			if (Y) {
				R = function(ag, ah) {
					J(b.select("font,span", Y), function(aj) {
						var ai, ak;
						if (aj.face == X || aj.style.fontFamily == X) {
							ak = aj.firstChild;
							ad(aj);
							T = b.createRng();
							T.setStart(ak, ak.nodeValue.length);
							T.setEnd(ak, ak.nodeValue.length);
							n.setRng(T);
							ag.nodeChanged();
							ae()
						}
